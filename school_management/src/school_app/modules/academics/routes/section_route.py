...
def create_section_route(data: SectionCreateRequest):
    section = create_section(data, actor_id=g.user.id)
    serialized = SectionResponse.model_validate(section).model_dump()
...
def get_all_sections_route():
    ...
    result = get_all_sections(
        level_id=level_id, search=search, include_inactive=include_inactive, page=page, per_page=per_page
    )

    return jsonify({
        "items": [SectionResponse.model_validate(item).model_dump() for item in result.items],
        "page": result.page,
        "pages": result.pages,
        "total": result.total,
    }), 200
...
@section_bp.route("/<int:section_id>/edit", methods=["PUT", "PATCH"])
@role_required(Role.ADMIN)
@validate_request(SectionUpdateRequest)
def update_section_route(data: SectionUpdateRequest, section_id):
    section = update_section(data, section_id, actor_id=g.user.id)
    if section is None:
        abort(404, description="Section not found")
...
def delete_section_route(section_id):
    deleted = delete_section(section_id, actor_id=g.user.id)
    if not deleted:
        abort(404, description="Section not found")
