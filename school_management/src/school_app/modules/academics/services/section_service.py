...
def create_section(data, actor_id):
    ...
    if level is None:
        abort(404, description=f"Academic level with ID {data.level_id} not found.")
    ...
    try:
        ...
        db.session.rollback()
        abort(400, description="Could not create section — check for duplicate name within this level.")
    ...
    create_audit_log(
        ...
        description=f"Created section {section.name} under level {level.name}",
    )
...
def get_all_sections(level_id=None, search="", include_inactive=False, page=1, per_page=10):
    ...
    stmt = stmt.order_by(Section.display_order.asc(), Section.id.asc())
...
def update_section(data, section_id, actor_id):
    ...
    if data.name is not None and data.name != section.name:
        changes["name"] = {"before": section.name, "after": data.name}
        section.name = data.name
    if data.display_order is not None and data.display_order != section.display_order:
        changes["display_order"] = {"before": section.display_order, "after": data.display_order}
        section.display_order = data.display_order
    if data.is_active is not None and data.is_active != section.is_active:
        changes["is_active"] = {"before": section.is_active, "after": data.is_active}
    ...
    try:
        ...
        db.session.rollback()
        abort(400, description="Could not update section — check for duplicate name within this level.")
    ...
    if section.classrooms:
        abort(
            ...
            description=(
                ...
                f"{len(section.classrooms)} classroom(s) assigned to it. Reassign those first."
            ),
        )
