...
class AcademicStageInput(BaseModel):
    ...
    display_order: Optional[int] = None
    levels: List[AcademicLevelInput] = Field(
        default_factory=list,
    )


class AcademicStructureStepRequest(BaseModel):
    stages: List[AcademicStageInput] = Field(
        ...,
        min_length=1,
    )
...
class GradingConfigStepRequest(BaseModel):
    ...
    rules: List[GradingRuleInput] = Field(
        ...,
        min_length=1,
    )
