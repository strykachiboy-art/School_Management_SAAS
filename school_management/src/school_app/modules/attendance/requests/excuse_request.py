...
class ExcuseCreateRequest(BaseModel):
    attendance_id: int = Field(
        ...,
        gt=0,
        description="ID of the attendance record being excused",
    )
    reason: str = Field(
        ...,
        min_length=5,
...
class ExcuseUpdateRequest(BaseModel):
    reason: str = Field(
        ...
        min_length=5,
        max_length=500,
        description="Updated reason for the absence",
    )


class BulkExcuseReviewRequest(BaseModel):
    excuse_ids: list[int] = Field(
        ...,
        min_length=1,
        description="IDs of excuses to approve or reject",
    )
