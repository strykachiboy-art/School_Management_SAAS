...
def _count_failed_subjects(
    student_id, academic_session_id, min_subject_score, school_id
):
    stmt = (
        db.select(Result)
    ...
    return sum(
        1 for score in subject_scores.values() if score < min_subject_score
    )
...
def _find_classroom_for_level(current_classroom, target_level_id, school_id):
    ...
    if (
        current_classroom is not None
        and current_classroom.section_id is not None
        and current_classroom.section
    ):
...
def evaluate_student_promotion(student_id, academic_session_id, school_id=None):
    ...
    current_classroom = (
        db.session.get(Classroom, student.classroom_id)
    ...
    if rule is not None:
        ...
        if (
            rule.min_average_score is not None
            and average_score < rule.min_average_score
        ):
            checks_passed = False
        if (
            rule.min_attendance_percentage is not None
            and attendance_percentage < rule.min_attendance_percentage
        ):
            checks_passed = False
        if (
            rule.max_failed_subjects is not None
            and rule.min_subject_score is not None
        ):
            failed_subjects = _count_failed_subjects(
                student_id,
                academic_session_id,
                rule.min_subject_score,
                school_id,
            )
    ...
    if not passed:
        recommendation = PromotionDecision.REPEATED
    elif current_classroom is not None and getattr(
        current_classroom, "is_final_level", False
    ):
        recommendation = PromotionDecision.GRADUATED
    else:
        recommendation = PromotionDecision.PROMOTED
    ...
    if rule is not None:
        ...
        if expected_to_level_id != actual_to_level_id and not allow_level_skip:
            expected_name = (
                rule.to_level.name
                if rule.to_level
                else "graduation (no further level)"
            )
            actual_name = (
                to_level.name
                if to_level is not None
                else "an unrecognized/unleveled classroom"
            )
            raise ValueError(
        ...
        if (
            ...
            and from_level_int is not None
            and to_level_int is not None
        ):
...
def get_student_promotion_history(student_id, school_id=None, page=1, per_page=20):
    student = db.session.get(Student, student_id)
    if student is None:
        return None
...
def get_session_promotions(
    academic_session_id, school_id, decision=None, classroom_id=None
):
    session = db.session.get(AcademicSession, academic_session_id)
    if session is None or session.school_id != school_id:
        return None

    stmt = db.select(PromotionHistory).where(
...
def _find_next_classroom(current_classroom, school_id):
    ...
    stmt = db.select(Classroom).where(
        Classroom.level == target_level, Classroom.school_id == school_id
    )
...
def promote_session_students(
    ...
    query = db.select(Student).where(
        Student.classroom_id.isnot(None), Student.school_id == school_id
    )
    ...
    for student in students:
        evaluation = evaluate_student_promotion(
            student.id, academic_session_id, school_id
        )
        if evaluation is None:
            results["skipped"].append(
                {"student_id": student.id, "reason": "evaluation unavailable"}
            )
            continue
        ...
        if decision == PromotionDecision.REPEATED:
            ...
            if target_level_id is not None:
                next_classroom = _find_classroom_for_level(
                    current_classroom, target_level_id, school_id
                )
            else:
                next_classroom = _find_next_classroom(
                    current_classroom, school_id
                )
                ...
                results["skipped"].append(
                    {
                        ...
                        "reason": "no next-level classroom configured",
                    }
                )
                continue
            ...
            results["skipped"].append(
                {
                    "student_id": student.id,
                    "reason": f"unhandled decision {decision}",
                }
            )
            continue
