...
def _make_invoice(db_session, admin_actor_id, classroom, academic_session, term, student, amount=50000.0):
    ...
    create_fee_structure(payload_struct, actor_id=admin_actor_id)
    ...
    invoices = generate_invoices_for_term(payload_gen, actor_id=admin_actor_id)
    return invoices[0]
...
def test_initiate_gateway_payment_creates_pending_payment(
    app, db_session, admin_actor_id, classroom, academic_session, term, student
):
    invoice = _make_invoice(db_session, admin_actor_id, classroom, academic_session, term, student)
    ...
    with patch("school_app.modules.school_fees.services.school_fees_service.get_gateway", return_value=mock_gateway) as mock_get_gateway:
        result = initiate_gateway_payment(_gateway_payload(invoice))
...
def test_initiate_gateway_payment_exceeds_balance(
    ...
    with patch("school_app.modules.school_fees.services.school_fees_service.get_gateway"):
        with pytest.raises(ValueError):
            initiate_gateway_payment(_gateway_payload(invoice, amount=invoice.total_amount + 1))
...
def test_initiate_gateway_payment_marks_failed_on_gateway_error(
    ...
    with patch("school_app.modules.school_fees.services.school_fees_service.get_gateway", return_value=mock_gateway):
        with pytest.raises(ValueError):
            initiate_gateway_payment(_gateway_payload(invoice))
...
def test_confirm_gateway_payment_applies_to_invoice(
    ...
    with patch("school_app.modules.school_fees.services.school_fees_service.get_gateway", return_value=verify_gateway):
        confirmed = confirm_gateway_payment(reference)
...
def test_confirm_gateway_payment_is_idempotent(
    ...
    with patch("school_app.modules.school_fees.services.school_fees_service.get_gateway", return_value=verify_gateway):
        ...
        confirm_gateway_payment(reference)  # webhook fires again
...
def test_confirm_gateway_payment_reference_not_found(app, db_session):
    with pytest.raises(Exception):
        confirm_gateway_payment("SCH-9999-000000")