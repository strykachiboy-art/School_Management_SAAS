...
def test_create_fee_structure(app, db_session, admin_actor_id, classroom, academic_session, term):
    ...
    structure = create_fee_structure(payload, actor_id=admin_actor_id)
    
    assert structure is not None
    assert structure.amount == 50000.0
    assert structure.category == FeeCategory.TUITION

def test_generate_invoices_for_term(app, db_session, admin_actor_id, classroom, academic_session, term, student):
    ...
    invoices = generate_invoices_for_term(payload, actor_id=admin_actor_id)
    
    assert len(invoices) >= 1
    assert invoices[0].total_amount == 50000.0
    assert invoices[0].status == InvoiceStatus.UNPAID

def test_record_payment_success(app, db_session, admin_actor_id, classroom, academic_session, term, student):
    student.classroom_id = classroom.id
    db.session.merge(student)
    db.session.commit()
    ...
    payment = record_payment(payload, recorded_by=admin_actor_id)
...
def test_mark_overdue_invoices(app, db_session, admin_actor_id, classroom, academic_session, term, student):
    student.classroom_id = classroom.id
    db.session.merge(student)
    db.session.commit()
    ...
    count = mark_overdue_invoices(as_of=date.today())
